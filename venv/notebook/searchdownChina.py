...
#             if abc.get_up(conn, days, fday) is True:
#                 print(str(abc.stock))
#                 hk = hk + 1
...
def search_down_days(days,fday):
        with open('../tks/SZtks') as f:
            lines = f.read().splitlines()
        sz = 0
        for line in lines:
            ...
            if abc.get_down(conn, days, fday) is True:
                print(str(abc.stock))
                sz = sz + 1
        print(str(sz) + " sz stocks met requirements")
